...
const transformKhorosData = (khorosResponse, options = {}) => {
  ...
  const transformedPosts = messages
    ...
    .map(msg => {
      ...
      // NOTE: LSI Data Export API may not include these fields - they're often empty
      let topicId = String(msg.topicId || '').trim()
      let messageId = String(msg.messageId || '').trim()
      
      // If IDs are empty, try to extract from URL if available
      // Khoros URLs have format: /t5/board/topic/m-p/MESSAGE_ID or /t5/board/topic/td-p/TOPIC_ID
      if ((!messageId || messageId === '') && msg.url) {
        const urlMatch = msg.url.match(/\/m-p\/(\d+)/) || msg.url.match(/\/td-p\/(\d+)/)
        if (urlMatch && urlMatch[1]) {
          if (msg.url.includes('/m-p/')) {
            messageId = urlMatch[1]
          } else if (msg.url.includes('/td-p/')) {
            topicId = urlMatch[1]
          }
        }
      }
